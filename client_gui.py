...
class ConnectFrame(tk.Frame):
    ...
    def open_chat_win(self, event=None):
        ...
        self.chat_canvas.bind("<Button-4>", self.move_text_up)
        self.chat_canvas.bind("<Button-5>", self.move_text_down)
        self.message_ent = tk.Entry(self.chat_canvas, width=48)
        self.message_ent.place(x=10, y=750)
        frame = tk.Frame(self.chat_canvas, bd=1, bg='#9932CC', width=100, height=50)
        frame.place(x=self.message_ent.winfo_x()+self.message_ent.winfo_width(),
                            y=self.message_ent.winfo_y()-6)
        send_but_canvas = tk.Canvas(frame, bg='#9932CC')
        send_but_canvas.place(x=-10, y=-10)
        send_but_canvas.create_image((10, 10), anchor='nw', image=send_img, tags='send_but')
        send_but_canvas.tag_bind('send_but', '<Button-1>', self.send_messages)
        self.chat_canvas.pack()

    def start_chat_as_thread(self, event):
        if self.server_host_entry.get() and self.server_port_entry.get() and self.nickname_ent.get() and check_access():
    ...
    def start_chat(self):
        ...
        def receive_messages():
            ...
            while True:
                ...
                self.messages[str(message_id)] = [[10, mes_y], height, line_break_count]
        ...
        for tag in self.moved_messages.keys():
            ...
            for item in items:
                ...
                if coords:  # Если координаты существуют
                    # Предположим, что coords содержит [x, y], обновим y-координату
                    current_y = coords[1]  # Получаем текущее значение y
                    new_y = current_y - 100 # Ваше смещение по y
                    self.chat_canvas.coords(item, coords[0], new_y)  # Обновляем координаты
                    self.chat_canvas.update()
                    self.moved_messages[tag][0] = new_y
